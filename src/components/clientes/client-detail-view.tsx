...
function buildAutoContentEvents(
  ...
  function pushEvent(day: number, type: ContentEvent["type"]) {
    const scheduledDate = `${y}-${String(m).padStart(2, "0")}-${String(day).padStart(2, "0")}T08:00`;
    events.push({
      // Nace "SIN EDITAR": pasa a CALENDARIZADO cuando se marca a mano en Planificación
      // o con el +/- del contador (que es un espejo, no un número libre).
      clientId, title: "", type, status: "SIN EDITAR", scheduledDate,
  ...
  if (counts.historias > 0) {
    for (let day = 1; day <= daysInMonth; day++) pushEvent(day, "HISTORIA");
  }
...
function ClientDataModal({
  ...
  // "Hecho" ya no se tipea: es un espejo de cuántos eventos de ese tipo están CALENDARIZADO este mes.
  const currentValues = {
    historiasHechas: countCalendarizadoByType(contentEvents, lead.id, monthKey, "HISTORIA"),
    historiasContratadas: monthlyRecord?.historiasContratadas ?? prefillContratado.historiasContratadas,
    reelsHechos: countCalendarizadoByType(contentEvents, lead.id, monthKey, "REEL"),
    reelsContratados: monthlyRecord?.reelsContratados ?? prefillContratado.reelsContratados,
    publicacionesHechas: countCalendarizadoByType(contentEvents, lead.id, monthKey, "PLACA"),
  ...
  function runAutoGenerate(values: typeof currentValues) {
    ...
    buildAutoContentEvents(lead.id, monthKey, {
      historias: values.historiasContratadas,
      reels: values.reelsContratados,
      publicaciones: values.publicacionesContratadas,
                  ...
                  <div key={label} className="contenidos-row">
                    ...
                    <div className="contenidos-row-inputs">
                      ...
                      <input
                        ...
                        value={hecho || ""}
                        readOnly
                        title="Se actualiza solo: cuenta los contenidos marcados como CALENDARIZADO en Planificación este mes"
                        style={{ background: "var(--slate-50, #f8fafc)", color: "#64748b", cursor: "default" }}
                      />
