...
/** Formatea un ticket como "350.000" (miles) o "350.000,50" (con decimales) */
function formatTicket(n: number | undefined): string {
  if (n === undefined || n === null || Number.isNaN(n)) return "";
  return n.toLocaleString("es-AR", { maximumFractionDigits: 2 });
}

/** Parsea "350.000,50" (formato es-AR) a 350000.5 */
function parseTicket(s: string): number | undefined {
...
/* ── Auto-generación de calendario desde el contador de Contenidos ───── */

/** Intercala varios tipos de contenido lo más parejo posible según su cantidad. */
function interleaveTypes(items: { type: ContentEvent["type"]; count: number }[]): ContentEvent["type"][] {
  const total = items.reduce((s, it) => s + it.count, 0);
  const progress = items.map(() => 0);
  const seq: ContentEvent["type"][] = [];
  for (let i = 0; i < total; i++) {
    let bestIdx = -1, bestRatio = Infinity;
    items.forEach((it, idx) => {
      if (it.count === 0) return;
      const ratio = progress[idx] / it.count;
      if (ratio < bestRatio) { bestRatio = ratio; bestIdx = idx; }
    });
    seq.push(items[bestIdx].type);
    progress[bestIdx]++;
  }
  return seq;
}
...
/**
 * Genera los eventos de contenido del mes a partir de lo contratado (número fijo, de la derecha
 * del contador). Historias Diarias = una por cada día del mes (siempre a las 8:00, se repite
 * "de lunes a lunes"). Reels y Publicaciones se intercalan entre sí a lo largo del mes, también
 * a las 8:00. Todos con título vacío ("Sin título") para completar a mano después.
 *
 * Solo se marcan CALENDARIZADO tantos como indica "hecho" (izquierda) — el resto queda SIN
 * EDITAR. Reels/Publicaciones: los primeros `hecho` de cada tipo, en orden de fecha. Historias
 * es todo-o-nada (no hay forma de repartir "3 de 7 días" sin inventar un criterio): CALENDARIZADO
 * completo solo cuando hecho ya alcanzó lo contratado.
 */
function buildAutoContentEvents(
  clientId: string,
  monthKey: string,
  counts: { historias: number; reels: number; publicaciones: number },
  hechos: { historias: number; reels: number; publicaciones: number },
): Omit<ContentEvent, "id" | "order">[] {
  const [y, m] = monthKey.split("-").map(Number);
  const daysInMonth = new Date(y, m, 0).getDate();
  const events: Omit<ContentEvent, "id" | "order">[] = [];

  function pushEvent(day: number, type: ContentEvent["type"], calendarizado: boolean) {
    const scheduledDate = `${y}-${String(m).padStart(2, "0")}-${String(day).padStart(2, "0")}T08:00`;
    events.push({
      clientId, title: "", type, status: calendarizado ? "CALENDARIZADO" : "SIN EDITAR", scheduledDate,
      done: false, timerSeconds: 0, timerRunning: false,
      autoGenerated: true,
    });
  }

  if (counts.historias > 0) {
    const historiasCompletas = hechos.historias >= counts.historias;
    for (let day = 1; day <= daysInMonth; day++) pushEvent(day, "HISTORIA", historiasCompletas);
  }

  const totalReelsPublis = counts.reels + counts.publicaciones;
  if (totalReelsPublis > 0) {
    const seq = interleaveTypes([
      { type: "REEL", count: counts.reels },
      { type: "PLACA", count: counts.publicaciones },
    ]);
    let reelsMarcados = 0, publisMarcadas = 0;
    seq.forEach((type, i) => {
      const day = Math.min(daysInMonth, 1 + Math.floor((i * daysInMonth) / totalReelsPublis));
      let calendarizado: boolean;
      if (type === "REEL") { calendarizado = reelsMarcados < hechos.reels; reelsMarcados++; }
      else { calendarizado = publisMarcadas < hechos.publicaciones; publisMarcadas++; }
      pushEvent(day, type, calendarizado);
    });
  }
...
function ClientDataModal({
  ...
  const [showClaveFacebook, setShowClaveFacebook] = useState(false);
  ...
  const CONTRATADO_KEYS: (keyof typeof currentValues)[] = ["historiasContratadas", "reelsContratados", "publicacionesContratadas"];

  function runAutoGenerate(values: typeof currentValues) {
    const { contentEvents, addContentEvent, deleteContentEvent } = useContentEventsStore.getState();
    const monthPrefix = monthKey; // "YYYY-MM"
    contentEvents
      .filter((e) => e.clientId === lead.id && e.autoGenerated && (e.scheduledDate ?? "").startsWith(monthPrefix))
      .forEach((e) => deleteContentEvent(e.id));
    buildAutoContentEvents(
      lead.id, monthKey,
      { historias: values.historiasContratadas, reels: values.reelsContratados, publicaciones: values.publicacionesContratadas },
      { historias: values.historiasHechas, reels: values.reelsHechos, publicaciones: values.publicacionesHechas },
    ).forEach((e) => addContentEvent(e));
  }

  function scheduleAutoGenerate(values: typeof currentValues) {
    if (autoGenerateTimeoutRef.current) clearTimeout(autoGenerateTimeoutRef.current);
    autoGenerateTimeoutRef.current = setTimeout(() => runAutoGenerate(values), 700);
  }

  function updateContenidoField(key: keyof typeof currentValues, value: number) {
    const nextValues = { ...currentValues, [key]: value };
    onUpdateMonthly(nextValues);
    if (CONTRATADO_KEYS.includes(key)) scheduleAutoGenerate(nextValues);
  }

  function handleGenerateClick() {
    runAutoGenerate(currentValues);
  ...
  return (
    ...
    <div className="modal-backdrop open" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body client-data-grid" style={{ overflowY: "auto", padding: "16px 24px" }}>

          <div className="field-group" style={{ gridColumn: "1 / -1", flexDirection: "row", justifyContent: "center", marginBottom: 2 }}>
            <LogoUploader leadId={lead.id} logoUrl={lead.logoUrl} onUploaded={(url) => onUpdate({ logoUrl: url })} size={56} />
          </div>
          ...
          <div className="field-group">
            ...
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
              ...
              <input
                ...
                type="text"
                inputMode="decimal"
                value={formatTicket(lead.ticket)}
                onChange={e => onUpdate({ ticket: parseTicket(e.target.value) })}
                placeholder="0"
          ...
          <div className="field-group">
            ...
            <input type="date" className="field" value={lead.fechaInicio ?? ""} onChange={e => onUpdate({ fechaInicio: e.target.value })} />
          </div>
          <div className="field-group">
          ...
          {/* Facebook + clave Facebook */}
          <div className="field-group">
            <label className="field-label">Facebook</label>
            <input className="field" value={lead.facebook ?? ""} onChange={e => onUpdate({ facebook: e.target.value })} placeholder="Usuario o página" />
          </div>
          <div className="field-group">
            <label className="field-label">Clave Facebook</label>
            <div style={{ position: "relative" }}>
              <input type={showClaveFacebook ? "text" : "password"} className="field" value={lead.claveFacebook ?? ""} onChange={e => onUpdate({ claveFacebook: e.target.value })} placeholder="Contraseña" style={{ paddingRight: 38 }} />
              <button type="button" onClick={() => setShowClaveFacebook(v => !v)} title={showClaveFacebook ? "Ocultar" : "Mostrar"} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#94a3b8", padding: 0, display: "flex", alignItems: "center" }}>
                {showClaveFacebook ? (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>)
                : (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>)}
              </button>
            </div>
          ...
          <div className="field-group" style={{ gridColumn: "1/-1" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <label className="field-label" style={{ marginBottom: 0 }}>Contenidos</label>
              <button
                type="button"
                className="btn btn-sm btn-outline"
                onClick={handleGenerateClick}
                title="Genera/actualiza en Planificación los Reels, Publicaciones e Historias de este mes según lo contratado"
                style={justGenerated ? { color: "#16a34a", borderColor: "#16a34a" } : undefined}
              >
                {justGenerated ? "✓ Calendario generado" : "↻ Generar calendario"}
              </button>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8, marginTop: 6 }}>
