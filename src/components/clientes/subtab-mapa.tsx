...
function pinPos(lead: Lead) {
  ...
  const x = 6 + hash(seed + "x", 86);
  const y = 8 + hash(seed + "y", 78);
  return { x, y };
}
...
export function SubtabMapa() {
  ...
  const pins = useMemo(
    () =>
      rows
        .filter((r) =>
          ["CLIENTES", "SEGUIMIENTO", "REUNION_1", "REUNION_2"].includes(r.tab)
        )
        .map((lead, idx) => ({
          lead,
          idx: idx + 1,
    ...
    <section className="map-panel-v30">
      ...
      <div className="map-canvas-v30">
        {pins.map(({ lead, idx, x, y, color }) => {
          const title = `${getTitle(lead)} · ${lead.direccion || "Sin dirección"} · ${lead.tab}`;
      ...
      <div className="map-legend-v30">
        {LEGEND.map((l) => (
          <span key={l.label} style={{ borderLeft: `4px solid ${l.color}` }}>
            {l.label}
          </span>
        ))}
      </div>
