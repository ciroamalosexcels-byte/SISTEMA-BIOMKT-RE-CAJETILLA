...
function CajaRow({ label, value, onChange, amber }: { label: string; value: number; onChange: (n: number) => void; amber?: boolean }) {
  ...
  return (
    <div className={`flex items-center flex-1 ${amber ? "bg-amber" : "border-b border-slate-100 dark:border-white/[0.04]"}`}>
      <div
        className={`py-[6px] text-[12px] font-black uppercase tracking-[0.06em] border-r border-slate-200 dark:border-white/[0.06] flex-shrink-0 ${amber ? "text-[#07152f]" : "text-slate-500 dark:text-slate-400"}`}
    ...
    <Card title={`Clientes ${mesLabel}`}>
      <div className="flex flex-col divide-y divide-slate-100 dark:divide-white/[0.04]">
        ...
        <div className="flex items-center justify-between pl-5 pr-[31px] py-[9px]">
          <span className="text-[12px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.06em]">Total clientes activos</span>
        ...
        <div className="flex items-center justify-between px-5 py-[9px]">
          <div className="flex items-center gap-2">
            <span className="text-[12px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.06em]">Objetivo clientes</span>
            {objetivo > 0 && (
        ...
        <div className="flex items-center justify-between px-5 py-[9px]">
          <div className="flex flex-col">
            <span className="text-[12px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.06em]">Ticket promedio</span>
        ...
        <div className="flex items-center justify-between pl-5 pr-[31px] py-[9px]">
          <div className="flex flex-col">
            <span className="text-[12px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.06em]">Conversión a clientes</span>
    ...
    <div className="bg-white dark:bg-[#0b1628] border border-slate-200 dark:border-white/[0.06] rounded-[18px] overflow-hidden">
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr 2fr 3fr" }}>
        {sections.map(({ key, label }, i) => (
          <div key={key} className={`px-4 py-2.5 bg-[#07152f] text-[13px] font-black text-white uppercase tracking-[0.02em] ${i > 0 ? "border-l border-white/[0.1]" : ""}`}>
            {label}
          </div>
        ))}
      </div>
