...
### Responsables y colores del sistema en Calendario de gestión (`/clientes/[id]`)
`ManagementEvent` tiene `responsables?: string[]` (IDs de `TeamMember`) — columna `responsables text[]` en `management_events` (Supabase), serializada en `src/lib/supabase/serializers.ts` → `serializeManagementEvent` y mapeada en el GET de `src/app/api/supabase/management-events/route.ts`. `ClientCalendarCard` (`client-detail-view.tsx`) acepta `fullLabel` (muestra `ev.type` completo en vez de la abreviación de `MGMT_ABBREV`, solo usado por el calendario de gestión — el de contenido sigue abreviado) y `memberInfo: Map<id, {nombre, color}>`. Click en el badge de un evento despliega inline (dentro de la misma celda) la lista de responsables sin abrir `ManagementDayModal`; click en el día vacío sigue abriendo el modal normal. `ManagementDayModal` tiene un selector multi-chip de integrantes activos en "Agregar al día". Paleta fija `MEMBER_COLOR_PALETTE` (rojo/verde/azul/amarillo-biomarketing/violeta/cyan/naranja/rosa) asigna color por índice de cada integrante en `useTeamStore().members` — mismo criterio usado para `MGMT_TYPE_COLOR` (colores por tipo de evento) y para las pastillas de responsables. `TeamMember.color` (columna ya existente en `team_members`) no se usa para esto — no hay UI para editarlo.

### Filtro por tipeo (type-ahead) en Clientes (`/clientes`)
`ClientesView` (`clientes-view.tsx`) escucha `keydown` a nivel `window` (ignora si el foco está en input/textarea/contentEditable o si hay modificador ctrl/alt/meta apretado). Cada letra o número tipeado se acumula en `searchBuffer`; si pasan más de `SEARCH_TYPE_WINDOW_MS` (3000 ms) sin tipear, la siguiente tecla reinicia el buffer en vez de agregarse al anterior (se trackea con `lastKeyTimeRef`, sin timers). Filtra `activos`/`inactivos` con `.toLowerCase().includes()` sobre `empresa || nombre`. Mientras el filtro está activo se agrega una tarjeta `ClearSearchCard` (mismo tamaño y forma que `ClientCard`) al final de la última grilla no vacía — o sola, si no hay resultados —; click en ella limpia el filtro y restaura el orden original. `Backspace` también limpia el filtro completo (no borra de a un carácter).

### Tarjetas de Equipo rediseñadas + drag and drop (`/equipo`)
`TeamMember.teamOrder` (columna `team_order` en `team_members`) persiste el orden — mismo patrón drag-and-drop que `ClientCard` en Clientes, implementado en `EquipoView`/`MemberCard` (`equipo-view.tsx`). La tarjeta ya no tiene los botones "Inactivar"/"Eliminar" ni las medallas: esas acciones se movieron al pie de `DatosModal` (`onToggleActivo`/`onDelete`, solo wireado desde `member-profile.tsx`, no desde la creación de un integrante nuevo). En su lugar la tarjeta muestra, en un renglón compacto con íconos SVG propios (no emoji — `CakeIcon`/`ClockIcon`/glifo zodiacal con selector de texto U+FE0E): edad, horario y signo. El badge junto al nombre (antes sueldo) ahora muestra `member.roles` en violeta (`bg-bviolet/[0.12] text-bviolet` — mismo token que usa el sidebar). Arriba a la derecha hay un círculo relleno (no aro) coloreado según el promedio de status 9.1 (`s91AverageColor`).
